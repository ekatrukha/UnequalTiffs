...
public class UTExploreBDV < T extends RealType< T > & NativeType< T > >
{
	...
	ArrayList<BdvStackSource< ? >> bdv_sources = new ArrayList<BdvStackSource< ? >>();
	...
	public void browseBDV(boolean bIs2D)
	{
		...
		for(int i=0;i<nImgN;i++)
		{
			...
			if(!imageSet.bMultiCh)
			{
				...
				{
					bdv_sources.add(BdvFunctions.show(intervals.get(i), imageSet.sFileNamesShort[i], Bdv.options().addTo(Tempbdv).sourceTransform(arrTr)));
				}
